...
Clarinet.test({
    name: "Ensure parametric-insurance-engine can be initialized",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get('deployer')!;
        
        let block = chain.mineBlock([
            Tx.contractCall('parametric-insurance-engine', 'initialize', [], deployer.address)
        ]);
        ...
        let statusBlock = chain.mineBlock([
            Tx.contractCall('parametric-insurance-engine', 'get-contract-status', [], deployer.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let authBlock = chain.mineBlock([
            Tx.contractCall('parametric-insurance-engine', 'authorize-user', [types.principal(wallet1.address)], deployer.address)
        ]);
        assertEquals(authBlock.receipts[0].result.expectOk(), true);
        
        // Check authorization
        let checkBlock = chain.mineBlock([
            Tx.contractCall('parametric-insurance-engine', 'is-user-authorized', [types.principal(wallet1.address)], deployer.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let pauseBlock = chain.mineBlock([
            Tx.contractCall('parametric-insurance-engine', 'emergency-pause', [], deployer.address)
        ]);
