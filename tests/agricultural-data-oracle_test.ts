...
Clarinet.test({
    name: "Ensure agricultural-data-oracle can be initialized",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get('deployer')!;
        
        let block = chain.mineBlock([
            Tx.contractCall('agricultural-data-oracle', 'initialize', [], deployer.address)
        ]);
        ...
        let statusBlock = chain.mineBlock([
            Tx.contractCall('agricultural-data-oracle', 'get-contract-status', [], deployer.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let authBlock = chain.mineBlock([
            Tx.contractCall('agricultural-data-oracle', 'authorize-user', [types.principal(wallet1.address)], deployer.address)
        ]);
        assertEquals(authBlock.receipts[0].result.expectOk(), true);
        
        // Check authorization
        let checkBlock = chain.mineBlock([
            Tx.contractCall('agricultural-data-oracle', 'is-user-authorized', [types.principal(wallet1.address)], deployer.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let pauseBlock = chain.mineBlock([
            Tx.contractCall('agricultural-data-oracle', 'emergency-pause', [], deployer.address)
        ]);
